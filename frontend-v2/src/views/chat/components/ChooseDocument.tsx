...
const ChooseDocument:React.FC<ChooseDocument>=({chooseDocument,setChooseDocument,listDocument,currentDocument,file,url,setFile,setIsValidInput}) =>{
  ...
  return (
    <React.Fragment>
      ...
      >
        <DialogTitle sx={{
            color:"black",
        }}>Choose document to embedding</DialogTitle>
        <DialogContent>
          <DialogContentText
          sx={{
            color:"black"
          }}>
           Current in use: <span style={{ fontWeight: 'bold', color:"#008000" }}>{currentDocument}</span>
          </DialogContentText>
      ...
      {listDocument.map((item:string)=>{
        ...
        <DialogActions>
          ...
          <Button type="submit" onClick={handleEmbedding}>Confirm</Button>
        </DialogActions>
      </Dialog>
    </React.Fragment>
  );
